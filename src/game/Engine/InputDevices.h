...
/// These are direcly taken from SDL_scancode.h for 1:1 mapping
...
struct KeyboardState {
	...
	bool HasKey(KeyboardKey key) const { return std::find(keys.cbegin(), keys.cend(), key) != keys.cend(); }
};
...
class Keyboard {
private:
	KeyboardState prevState, currentState;
public:
	void Update();

	inline bool IsDown(KeyboardKey key) const { return currentState.HasKey(key); }
	inline bool IsPressed(KeyboardKey key) const { return !prevState.HasKey(key) && currentState.HasKey(key); }
	inline bool IsReleased(KeyboardKey key) const { return prevState.HasKey(key) && !currentState.HasKey(key); }
	inline bool IsAnyKeyDown() const { return currentState.keys.size() > 0; }
	inline bool IsAnyKeyPressed() const { return prevState.keys.size() == 0 && currentState.keys.size() > 0; }
	inline bool IsAnkyKeyReleased() const { return prevState.keys.size() > 0 && currentState.keys.size() == 0; }
};