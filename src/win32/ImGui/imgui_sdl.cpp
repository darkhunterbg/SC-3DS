...
namespace
{
	...
	struct Device
	{
		...
		struct TriangleCacheItem
		{
			...
			~TriangleCacheItem() { if (Texture) GPU_FreeImage(Texture); }
		};
		...
		Device(GPU_Target* renderer) : Renderer(renderer), Main(renderer) {}

		void SetClipRect(const ClipRect& rect)
		{
			Clip = rect;
			GPU_Rect clip = { (float)rect.X,(float)rect.Y, (float)rect.Width, (float)rect.Height };
			GPU_SetClipRect(Renderer, clip);
		}

		void EnableClip() { SetClipRect(Clip); }
		void DisableClip() { GPU_UnsetClip(Renderer); }

		void SetAt(int x, int y, const Color& color)
		{
			GPU_Pixel(Renderer, x, y, color.ASSDLColor());
		}

		GPU_Image* MakeTexture(int width, int height)
		{
			GPU_Image* texture = GPU_CreateImage(width, height, GPU_FORMAT_RGBA);
			GPU_SetBlendMode(texture, GPU_BLEND_NORMAL);
			return texture;
		}

		void UseAsRenderTarget(GPU_Image* texture)
		{
			if (texture == nullptr)
				Renderer = Main;
			else
	...
	void DrawTriangleWithColorFunction(const FixedPointTriangleRenderInfo& renderInfo, const std::function<Color(float x, float y)>& colorFunction, Device::TriangleCacheItem* cacheItem)
	{
		...
		GPU_Image* cache = CurrentDevice->MakeTexture(width, height);
	...
	{
		GPU_Rect destination = { (float) renderInfo.MinX, (float)renderInfo.MinY, (float)triangle.Width,(float)triangle.Height };
		GPU_BlitRect(triangle.Texture, nullptr, CurrentDevice->Renderer, &destination);
	}
	...
	void DrawTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3, const Texture* texture)
	{
		...
		GPU_Rect destination = { (float)renderInfo.MinX, (float)renderInfo.MinY,(float)cached->Width, (float)cached->Height };
		GPU_BlitRect(cached->Texture, nullptr, CurrentDevice->Renderer, &destination);
	...
	void DrawRectangle(const Rect& bounding, GPU_Image* texture, int textureWidth, int textureHeight, const Color& color, bool doHorizontalFlip, bool doVerticalFlip)
	{
		...
		GPU_Rect destination = {
			(float)(bounding.MinX),
			(float)(bounding.MinY),
			(float)(bounding.MaxX - bounding.MinX),
			(float)(bounding.MaxY - bounding.MinY)
		};

		// If the area isn't textured, we can just draw a rectangle with the correct color.
		if (bounding.UsesOnlyColor())
		{
			GPU_RectangleFilled2(CurrentDevice->Renderer, destination, color.ASSDLColor());
		}
		else
		{
			...
			GPU_Rect source = {
				(float)(bounding.MinU * textureWidth),
				(float)(bounding.MinV * textureHeight),
				(float)((bounding.MaxU - bounding.MinU) * textureWidth),
				(float)((bounding.MaxV - bounding.MinV) * textureHeight)
			};

			GPU_FlipEnum flip = static_cast<GPU_FlipEnum>((doHorizontalFlip ? SDL_FLIP_HORIZONTAL : 0) | (doVerticalFlip ? SDL_FLIP_VERTICAL : 0));

			GPU_SetColor(texture, color.ASSDLColor());

			// CHECK
			GPU_BlitRectX(texture, &source, CurrentDevice->Renderer, &destination,0, texture->w/2, texture->h/2, flip);
		}
	}
	...
	void DrawRectangle(const Rect& bounding, GPU_Image* texture, const Color& color, bool doHorizontalFlip, bool doVerticalFlip)
	{
		DrawRectangle(bounding, texture, texture->w, texture->h, color, doHorizontalFlip, doVerticalFlip);
	}
}
...
namespace ImGuiSDL
{
	void Initialize(GPU_Target* renderer)
	{
		...
		io.DisplaySize.x = renderer->w;
		io.DisplaySize.y = renderer->h;
	...
	/*	SDL_BlendMode blendMode;
		...
		GPU_Image* initialRenderTarget = SDL_GetRenderTarget(CurrentDevice->Renderer);*/
					...
					for (unsigned int i = 0; i + 3 <= drawCommand->ElemCount; i += 3)
					{
						...
						if (i + 6 <= drawCommand->ElemCount)
						{
								...
								&& bounding.IsOnExtreme(v5.pos))
							{
								...
								{
									DrawRectangle(bounding, static_cast<GPU_Image*>(drawCommand->TextureId), Color(v0.col), doHorizontalFlip, doVerticalFlip);
								}
